...
use App\Models\RdpExtendedModel;
use App\Models\RdpModel;
use App\Models\UsersModel;
use App\Models\SectionsModel;
use App\Models\NotificationsModel;
use monken\TablesIgniter;
class Rdp extends BaseController
{
		...
		if(session()->get("logedin") == '1'){
	    	...
	    	foreach($ids as $vals){
	    		...
	    		if(preg_match("/^([0-9])+$/i", $val) == true){
	    			$getProductInfos = $RdpModel->where(["id" => $val, ])->findAll();
	    			if(count($getProductInfos) > 0){
	    				$Checkresults = [];
						$doCheckRequ = rdpchecker($getProductInfos[0]['host'], $getProductInfos[0]['user'], $getProductInfos[0]['pass']);
						if($doCheckRequ == true){
							$good++;
    				...
    				if($countResults == 1){
    					...
    					if(count($getseller) > 0){
    						if($Results[0]['sellerid'] == $getseller[0]['id'] || session()->get('suser_groupe') == '9'){
    							$form = '
    								<form id="edittForm">
    									<div class="form-group row">
    										...
    										<div class="col-6">
    											<label>System</label>
    											<input type="text" id="system" name="system" class="form-control" value="'.$Results[0]['system'].'">
    											<small class="system text-danger"></small>
    										</div>
    									</div>
    									<div class="form-group row">
    										<div class="col-6">
    									...
    									<div class="form-group row">
    										<div class="col-6">
    											<label>RAM<i class="text-danger"> *</i></label>
    											<input type="text" id="ram" name="ram" class="form-control" value="'.$Results[0]['ram'].'">
    											<small class="ram text-danger"></small>
    										</div>
    										<div class="col-6">
    											<label>HDD Size<i class="text-danger"> *</i></label>
    											<input type="text" id="hddsize" name="hddsize" class="form-control" value="'.$Results[0]['hddsize'].'">
    											<small class="hddsize text-danger"></small>
    										</div>
    									</div>
    									<div class="form-group row">
    										<div class="col-12">
    							...
    							'; 
    							$modalContent = $form;
    							$response["modal"] = createModal($modalContent, 'fade  ', 'Edit', 'text-primary', 'modal-lg', "1", "1", "1", "1", "1", ['text' => 'Save', 'functions' => 'data-api="edititem-editrdp|'.$Results[0]['id'].'"']);
    						}
    						else {
	   ...
	   if($this->request->isAJAX()){ 
    		if(session()->get("suser_groupe") != "9" && session()->get("suser_groupe") != "1"){
    			...
    			$ValidationRulls = [
    				'host' => [
    					'label' => 'Host',
    		            'rules'  => 'required|regex_match[/^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/]',
    		            'errors' => [
    	            ...
    	            'system' => [
    	            	'label' => 'System',
    		            'rules'  => 'permit_empty|regex_match[/^[a-zA-Z0-9 \-\_]{2,30}$/]',
    		            'errors' => [
    		            	'regex_match' => 'Invalid System input.',
    	            	]
    	            ],
    	            'ram' => [
    	            	'label' => 'RAM',
    		            'rules'  => 'permit_empty|regex_match[/^[a-zA-Z0-9]{2,20}$/]',
    		            'errors' => [
    		            	'regex_match' => 'Invalid RAM input.',
    	            	]
    	            ],
    	            'hddsize' => [
    	            	'label' => 'Password',
    		            'rules'  => 'permit_empty|regex_match[/^[a-zA-Z0-9]{2,20}$/]',
    		            'errors' => [
    		            	'regex_match' => 'nvalid HDD Size input.',
    	            	]
    	            ],
    	            'price' => [
    	            	'label' => 'IP Address',
    			...
    			if(!$this->validate($ValidationRulls)){
    				...
    				if(preg_match("/^([0-9])+$/i", base64_decode($this->request->getPost('id')))){
    					...
    					if($countResults == 1){
							...
							try {
								if(filter_var($this->request->getPost('host'), FILTER_VALIDATE_IP) && $this->request->getPost('host') != '127.0.0.1'){
							        $json = file_get_contents('https://api.ipregistry.co/'.$this->request->getPost('host').'?key=pf0f4w9q20rzwwoz' ,JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
									...
									if(null !== $this->request->getPost('system')){
										$data["system"] = $this->request->getPost('system');
									} 
									else {
										$data["system"] = 'N/A';
									}
									if(null !== $this->request->getPost('ram')){
										$data["ram"] = $this->request->getPost('ram');
									}
									else {
										$data["ram"] = 'N/A';
									}
									if(null !== $this->request->getPost('hddsize')){
										$data["hddsize"] = $this->request->getPost('hddsize');
									}
									else {
										$data["hddsize"] = 'N/A';
									}
    		...
    		if(session()->get("suser_groupe") != "9" && session()->get("suser_groupe") != "1"){
    			...
    			if(preg_match("/^[0-9]{1,11}$/",base64_decode($this->request->getPost('id')))){
    				...
    				if($countResults == 1){
    					...
    					$response["modal"] = createModal($modalContent, 'fade', 'Delete Item', 'text-danger', 'modal-lg', "1", "1", "1", "1", "1",['text' => 'Delete', 'functions' =>'data-api="rm-rdp|'.base64_encode($Results[0]["id"]).'"']);
    				}
    				else {
    					$modalContent = '<p>Object not found. E002</p>';
    					$response["modal"] = createModal($modalContent, 'fade', 'Error', 'text-danger', 'modal-lg', "1", "1", "1", "1", "0");
	...
	public function massinitEdit(){
	    if($this->request->isAJAX()){
    		if(session()->get("suser_groupe") != "9" && session()->get("suser_groupe") != "1"){
    			...
    			'; 
    			$modalContent = $form;
    			$response["modal"] = createModal($modalContent, 'fade', 'Edit', 'text-primary', 'modal-lg', "1", "1", "1", "1", "1", ['text' => 'Save', 'functions' => 'data-api="massedits-rdp"']);
    			$response["csrft"] = csrf_hash();
    			echo json_encode($response);
    			exit();
	    ...
	    if($this->request->isAJAX()){
    		if(session()->get("suser_groupe") != "9" && session()->get("suser_groupe") != "1"){
				...
				if($countResults >= 2){
					...
					$response["modal"] = createModal($modalContent, 'fade', 'Delete Item', 'text-danger', 'modal-lg', "1", "1", "1", "1", "1",['text' => 'Delete', 'functions' =>'data-api="massrms-rdp"']);
				}
				else {
					$modalContent = '<p>Object not found. E002</p>';
					$response["modal"] = createModal($modalContent, 'fade', 'Error', 'text-danger', 'modal-lg', "1", "1", "1", "1", "0");
